...
function RunTest2([string]$test_cmd, [string[]]$export_types, [string[]]$inputs)
{
    $cmdParams = @("--sources=$headerSources","--sources=$coreSources","--modules=$buildDir","--working_dir=$buildDir")
...
RunTest2 $shared_lib_test ("binary:" + (Join-Path $buildDir "onnxruntime_shared_lib_test.cov"))


Write-Host "MLAS test"
$mlas_test = Join-Path $buildDir "onnxruntime_mlas_test.exe"
RunTest2 $mlas_test ("binary:" + (Join-Path $buildDir "onnxruntime_mlas_test.cov"))
...
RunTest2 $onnxruntime_test_all ("cobertura:$outputXml","html:$outputDir") ("onnxruntime_shared_lib_test.cov","onnx_test_runner.cov","onnxruntime_mlas_test.cov")
